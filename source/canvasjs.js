...
(function () {
	...
	var defaultOptions = {
		...
		Title: {
			...
			//agentTipContent: null//string - To be implemented (TBI)
		},
	...
	function Chart(containerId, options, publicChartReference) {
		...
		if (!isCanvasSupported) {
			this._canvasJSContainer.style.height = "0px";//In IE6 agentTip doesn't show at proper position if not set.
		}
		...
		this._agentBar = document.createElement("div");
		this._agentBar.setAttribute("class", "canvasjs-chart-agentbar");
		this._agentBar.style.cssText = "position: absolute; right: 1px; top: 1px;";
		this._canvasJSContainer.appendChild(this._agentBar);
		...
		this._agentTip = new AgentTip(this, this._options.agentTip, this.theme);
	...
	Chart.prototype._updateOptions = function () {
		...
		if (this._options.zoomEnabled) {
			...
			if (!this._zoomButton) {
				...
				this._agentBar.appendChild(this._zoomButton);
			...
			if (!this._resetButton) {
				...
				this._agentBar.appendChild(this._resetButton);

				addEvent(this._resetButton, "click", function () {

					_this._agentTip.hide();
		...
		if (this._menuButton) {
			...
			this._agentBar.appendChild(this._menuButton);
		...
		if (!this._dropdownMenu && this.exportEnabled && isCanvasSupported) {
			...
			this._agentBar.appendChild(this._dropdownMenu);
		...
		if (this._agentBar.style.display !== "none" && this._zoomButton) {
		...
		if (this._options.agentTip && this._agentTip._options !== this._options.agentTip)
			this._agentTip._options = this._options.agentTip

		for (var prop in this._agentTip._options) {

			if (this._agentTip._options.hasOwnProperty(prop)) {
				this._agentTip.updateOption(prop);
			}
		}
	...
	Chart.prototype.render = function (options) {
		...
		this._agentTip._updateAgentTip();
	...
	Chart.prototype.attachPlotAreaEventHandlers = function () {
		...
		//this._agentBar.style.display = "inline";
	...
	Chart.prototype._mouseEventHandler = function (ev) {
		...
		if (this._agentTip && this._agentTip.enabled) {
		...
		if ((!this.isDrag || !this.zoomEnabled) && this._eventManager) {
			...
			//this._updateAgentTip(ev.x, ev.y);            
		}

		//if (this._agentTip.enabled)
		//    this._agentTip.mouseMoveHandler(ev.x, ev.y);
	}
	...
	Chart.prototype._plotAreaMouseMove = function (x, y) {
		if (this.isDrag && this.plotInfo.axisPlacement !== "none") {
			...
			if (Math.abs(dragDelta) > 2 && Math.abs(dragDelta) < 8 && (this.panEnabled || this.zoomEnabled)) {
				...
				this._agentTip.mouseMoveHandler(x, y);
			}
			...
			//    this._agentTip.hide();
			//    return;
			//} else if (this._agentTip.enabled)
			//    this._agentTip.mouseMoveHandler(x, y);
			...
			for (var i = 0; i < dataPoints.length; i++) {
				...
				if (endAngle > startAngle) {
					...
					if (isInTransition) {
						...
						entry.index = i;
						_this._agentTip.highlightObjects([entry]);
					}
	...
	function AgentTip(chart, options, theme) {
		AgentTip.base.constructor.call(this, "AgentTip", options, theme);
	...
	extend(AgentTip, CanvasJSObject);

	AgentTip.prototype._initialize = function () {

		if (this.enabled) {
			this.container = document.createElement("div");
			this.container.setAttribute("class", "canvasjs-chart-agenttip");
			...
			var agentTipHtml = "<div style=\" width: auto;";
			agentTipHtml += "height: auto;";
			agentTipHtml += "min-width: 50px;";
			agentTipHtml += "line-height: auto;";
			agentTipHtml += "margin: 0px 0px 0px 0px;";
			agentTipHtml += "padding: 5px;";
			agentTipHtml += "font-family: Calibri, Arial, Georgia, serif;";
			agentTipHtml += "font-weight: normal;";
			agentTipHtml += "font-style: " + (isCanvasSupported ? "italic;" : "normal;");
			agentTipHtml += "font-size: 14px;";
			agentTipHtml += "color: #000000;";
			agentTipHtml += "text-shadow: 1px 1px 1px rgba(0, 0, 0, 0.1);";
			agentTipHtml += "text-align: left;";
			agentTipHtml += "border: 2px solid gray;";

			//Older browsers like IE8- don't support alpha values
			agentTipHtml += isCanvasSupported ? "background: rgba(255,255,255,.9);" : "background: rgb(255,255,255);";

			agentTipHtml += "text-indent: 0px;";
			agentTipHtml += "white-space: nowrap;";
			//agentTipHtml += "pointer-events:none;";
			agentTipHtml += "border-radius: 5px;";

			//Disable Text Selection
			agentTipHtml += "-moz-user-select:none;";
			agentTipHtml += "-khtml-user-select: none;";
			agentTipHtml += "-webkit-user-select: none;";
			agentTipHtml += "-ms-user-select: none;";
			agentTipHtml += "user-select: none;";

			//agentTipHtml += "opacity: 0;";
			//agentTipHtml += "filter: progid: DXImageTransform.Microsoft.gradient(GradientType = 0, startColorstr = '#4cffffff', endColorstr = '#4cffffff');";

			if (!isCanvasSupported) {
				//agentTipHtml += "-ms-filter:'progid:DXImageTransform.Microsoft.Alpha(Opacity=90)'";
				//-ms-filter: "progid:DXImageTransform.Microsoft.Shadow(Strength=4, Direction=135, Color='#000000')";
				/* For IE 5.5 - 7 */
				agentTipHtml += "filter: alpha(opacity = 90);";
				agentTipHtml += "filter: progid:DXImageTransform.Microsoft.Shadow(Strength=3, Direction=135, Color='#666666');";
			}

			agentTipHtml += "} \"> Sample Agenttip</div>";

			this.container.innerHTML = agentTipHtml;
	...
	AgentTip.prototype.mouseMoveHandler = function (x, y) {

		if (!(this._lastUpdated && (new Date().getTime() - this._lastUpdated) < 40)) {
			this._lastUpdated = new Date().getTime();
			this._updateAgentTip(x, y);
		}
	}

	AgentTip.prototype._updateAgentTip = function (mouseX, mouseY) {
		//return;

		if (this.chart.disableAgentTip) // Disabled during animation, etc
			return;
		...
		if (entries.length > 0) {
			...
			if (this.enabled) {
				...
				var agentTipInnerHtml = "";

				agentTipInnerHtml = this.getAgentTipInnerHTML({ entries: entries });

				if (agentTipInnerHtml !== null) {
					this.contentDiv.innerHTML = agentTipInnerHtml;
					...
					if (entries[0].dataSeries.type === "pie" || entries[0].dataSeries.type === "doughnut" || entries[0].dataSeries.type === "funnel" || entries[0].dataSeries.type === "bar" || entries[0].dataSeries.type === "rangeBar" || entries[0].dataSeries.type === "stackedBar" || entries[0].dataSeries.type === "stackedBar100") {
						agentTipLeft = mouseX - 10 - this.container.clientWidth;
					} else {
						agentTipLeft = (((this.chart.axisX.lineCoordinates.width / Math.abs(this.chart.axisX.maximum - this.chart.axisX.minimum)) * Math.abs(entries[0].dataPoint.x - this.chart.axisX.minimum)) + this.chart.axisX.lineCoordinates.x1 + .5) - this.container.clientWidth << 0;
						agentTipLeft -= 10;
					}


					if (agentTipLeft < 0) {
						agentTipLeft += this.container.clientWidth + 20;
					}

					if (agentTipLeft + this.container.clientWidth > this.chart._container.clientWidth)
						agentTipLeft = Math.max(0, this.chart._container.clientWidth - this.container.clientWidth);
					...
					if (entries.length === 1 && !this.shared && (entries[0].dataSeries.type === "line" || entries[0].dataSeries.type === "stepLine" || entries[0].dataSeries.type === "spline" || entries[0].dataSeries.type === "area" || entries[0].dataSeries.type === "stepArea" || entries[0].dataSeries.type === "splineArea" || entries[0].dataSeries.type === "stackedArea" || entries[0].dataSeries.type === "stackedArea100")) {
						agentTipBottom = (entries[0].dataSeries.axisY.lineCoordinates.y2 - entries[0].dataSeries.axisY.lineCoordinates.height / Math.abs(entries[0].dataSeries.axisY.maximum - entries[0].dataSeries.axisY.minimum) * Math.abs(entries[0].dataPoint.y - entries[0].dataSeries.axisY.minimum) + .5) << 0;
						...
						agentTipBottom = (entries[0].dataSeries.axisX.lineCoordinates.y2 - entries[0].dataSeries.axisX.lineCoordinates.height / Math.abs(entries[0].dataSeries.axisX.maximum - entries[0].dataSeries.axisX.minimum) * Math.abs(entries[0].dataPoint.x - entries[0].dataSeries.axisX.minimum) + .5) << 0;
					}
					else {
						agentTipBottom = mouseY;
					}

					agentTipBottom = (-agentTipBottom + 10);

					if (agentTipBottom + this.container.clientHeight + 5 > 0) {
						agentTipBottom -= agentTipBottom + this.container.clientHeight + 5 - 0
					}

					agentTipBottom += "px";

					//this.container.style.right = agentTipRight;
					this.container.style.left = agentTipLeft;
					this.container.style.bottom = agentTipBottom;
	...
	AgentTip.prototype.highlightObjects = function (entries) {
	...
	AgentTip.prototype.getAgentTipInnerHTML = function (e) {
		var entries = e.entries;
		var agentTipInnerHtml = null;
		var dataSeries = null;
		var dataPoint = null;
		var index = 0;
		var color = null;
		...
		var isAgentTipDefinedInData = true;
		for (var i = 0; i < entries.length; i++) {
			if (entries[i].dataSeries.agentTipContent || entries[i].dataPoint.agentTipContent) {
				isAgentTipDefinedInData = false;
				break;
			}
		}

		if (isAgentTipDefinedInData && ((this.content && typeof (this.content) === "function") || this.contentFormatter)) {

			var param = { chart: this.chart, agentTip: this._options, entries: entries };
			agentTipInnerHtml = this.contentFormatter ? this.contentFormatter(param) : this.content(param);

		} else {

			if (this.shared && this.chart.plotInfo.axisPlacement !== "none") {

				var agentTipInnerHtmlPrefix = "";
				...
				for (var i = 0; i < entries.length; i++) {
					...
					if (i === 0 && isAgentTipDefinedInData && !this.content) {
						agentTipInnerHtmlPrefix += typeof (this.chart.axisX.labels[dataPoint.x]) !== "undefined" ? this.chart.axisX.labels[dataPoint.x] : "{x}";
						agentTipInnerHtmlPrefix += "</br>";
						agentTipInnerHtmlPrefix = this.chart.replaceKeywordsWithValue(agentTipInnerHtmlPrefix, dataPoint, dataSeries, index);
					}

					//Allows disabling of agentTip for individual dataPoints/dataSeries
					if (dataPoint.agentTipContent === null || (typeof (dataPoint.agentTipContent) === "undefined" && dataSeries._options.agentTipContent === null))
						continue;
					...
					|| dataSeries.type === "stackedArea" || dataSeries.type === "stackedArea100") {
						agentTipContent += dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span style='\"'color:{color};'\"'>{name}:</span>&nbsp;&nbsp;{y}";
					}
					else if (dataSeries.type === "bubble") {
						agentTipContent += dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span style='\"'color:{color};'\"'>{name}:</span>&nbsp;&nbsp;{y}, &nbsp;&nbsp;{z}";
					} else if (dataSeries.type === "pie" || dataSeries.type === "doughnut" || dataSeries.type === "funnel") {
						agentTipContent += dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "&nbsp;&nbsp;{y}";
					} else if (dataSeries.type === "rangeColumn" || dataSeries.type === "rangeBar" || dataSeries.type === "rangeArea" || dataSeries.type === "rangeSplineArea") {
						agentTipContent += dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span style='\"'color:{color};'\"'>{name}:</span>&nbsp;&nbsp;{y[0]},&nbsp;{y[1]}";
					} else if (dataSeries.type === "candlestick" || dataSeries.type === "ohlc") {
						agentTipContent += dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span style='\"'color:{color};'\"'>{name}:</span>"
										+ "<br/>Open: &nbsp;&nbsp;{y[0]}"
										+ "<br/>High: &nbsp;&nbsp;&nbsp;{y[1]}"
										+ "<br/>Low:&nbsp;&nbsp;&nbsp;{y[2]}"
										+ "<br/>Close: &nbsp;&nbsp;{y[3]}";
					...
					if (this.reversed === true) {
						...
						agentTipInnerHtml = this.chart.replaceKeywordsWithValue(agentTipContent, dataPoint, dataSeries, index) + agentTipInnerHtml;

						if (i < entries.length - 1)
							agentTipInnerHtml = "</br>" + agentTipInnerHtml;

					} else {

						agentTipInnerHtml += this.chart.replaceKeywordsWithValue(agentTipContent, dataPoint, dataSeries, index);

						if (i < entries.length - 1)
							agentTipInnerHtml += "</br>";
				...
				if (agentTipInnerHtml !== null)
					agentTipInnerHtml = agentTipInnerHtmlPrefix + agentTipInnerHtml;
				...
				if (dataSeries.type === "line" || dataSeries.type === "stepLine" || dataSeries.type === "spline" || dataSeries.type === "area" || dataSeries.type === "stepArea" || dataSeries.type === "splineArea" || dataSeries.type === "column" || dataSeries.type === "bar" || dataSeries.type === "scatter"
					|| dataSeries.type === "stackedColumn" || dataSeries.type === "stackedColumn100" || dataSeries.type === "stackedBar" || dataSeries.type === "stackedBar100"
					|| dataSeries.type === "stackedArea" || dataSeries.type === "stackedArea100") {
					agentTipContent = dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span style='\"'color:{color};'\"'>" + (dataPoint.label ? "{label}" : "{x}") + " :</span>&nbsp;&nbsp;{y}";
				} else if (dataSeries.type === "bubble") {
					agentTipContent = dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span style='\"'color:{color};'\"'>" + (dataPoint.label ? "{label}" : "{x}") + ":</span>&nbsp;&nbsp;{y}, &nbsp;&nbsp;{z}";
				} else if (dataSeries.type === "pie" || dataSeries.type === "doughnut" || dataSeries.type === "funnel") {
					agentTipContent = dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : (dataPoint.name ? "{name}:&nbsp;&nbsp;" : dataPoint.label ? "{label}:&nbsp;&nbsp;" : "") + "{y}";
				} else if (dataSeries.type === "rangeColumn" || dataSeries.type === "rangeBar" || dataSeries.type === "rangeArea" || dataSeries.type === "rangeSplineArea") {
					agentTipContent = dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span style='\"'color:{color};'\"'>" + (dataPoint.label ? "{label}" : "{x}") + " :</span>&nbsp;&nbsp;{y[0]}, &nbsp;{y[1]}";
				} else if (dataSeries.type === "candlestick" || dataSeries.type === "ohlc") {
					agentTipContent = dataPoint.agentTipContent ? dataPoint.agentTipContent : dataSeries.agentTipContent ? dataSeries.agentTipContent : this.content && typeof (this.content) !== "function" ? this.content : "<span style='\"'color:{color};'\"'>" + (dataPoint.label ? "{label}" : "{x}") + "</span>"
						+ "<br/>Open: &nbsp;&nbsp;{y[0]}"
						+ "<br/>High: &nbsp;&nbsp;&nbsp;{y[1]}"
						+ "<br/>Low: &nbsp;&nbsp;&nbsp;&nbsp;{y[2]}"
						+ "<br/>Close: &nbsp;&nbsp;{y[3]}";
		...
		return agentTipInnerHtml;
	}

	AgentTip.prototype.enableAnimation = function () {
	...
	AgentTip.prototype.disableAnimation = function () {
		if (!this.container.style.WebkitTransition)
			return;
	...
	AgentTip.prototype.hide = function (resetOverlayedCanvas) {
		if (!this.enabled)
			return;
